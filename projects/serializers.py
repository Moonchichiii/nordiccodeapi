...
class ProjectPackageSerializer(serializers.ModelSerializer):
    """
    Serializer for project packages with custom validation and error handling.
    """

    # Removed display_name field as per requirements
    features = serializers.JSONField(required=True)
    tech_stack = serializers.ListField(child=serializers.CharField(), required=True)
    deliverables = serializers.ListField(child=serializers.CharField(), required=True)
    ...
    def validate_features(self, value):
        ...
        if isinstance(value, str):
            try:
                ...
                raise serializers.ValidationError(_("Invalid JSON format for features"))

        if not isinstance(value, dict):
            raise serializers.ValidationError(_("Features must be a dictionary"))
        return value
    ...
    def validate_tech_stack(self, value):
        ...
        if not isinstance(value, list):
            raise serializers.ValidationError(_("Tech stack must be a list"))
        return value

    def validate_base_price(self, value):
        """Validate base price"""
        if value <= 0:
            raise serializers.ValidationError(_("Base price must be greater than zero"))
        return value
...
class ProjectSerializer(serializers.ModelSerializer):
    """
    Serializer for projects with enhanced validation and error handling.
    """
    ...
    assigned_staff = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=User.objects.filter(is_staff=True),  # Only staff can be assigned
        required=False,
    )
    ...
    def to_representation(self, instance):
        ...
        if isinstance(instance, Project):  # Check if instance is a model instance
